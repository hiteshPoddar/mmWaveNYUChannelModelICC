...
class Utils
{
                                ...
                                Ipv4Address targetAddress, AppSetupParams params)
  {
    ...
    app1.Add (client.Install (srcNode));

    // Connect the TxWithSeqTsSize trace to the TxSinkHeader callback sink
    app1.Get(0)->TraceConnectWithoutContext ("TxWithSeqTsSize", MakeBoundCallback (&CallbackSinks::TxSinkHeader, 
                                                                                  traceStream));
    app1.Start (params.startTime);
    app1.Stop (params.endTime);
  }
...
main (int argc, char *argv[])
{
  ...
  AsciiTraceHelper asciiTraceHelper;
	...
	// Base ports
	uint16_t appBasePort = 49153; 		// base port for the UDP applications
	...
	std::string appRate = "10Mbps";	    // application data rate	
  ...
  AppSetupParams udpAppParams;
	...
	udpAppParams.IPI = MilliSeconds (50);		
  ...
  std::cout << "Remote host IP :" << remoteIface.GetAddress(1) << std::endl;
  ...
  flowMonitor->SerializeToXmlFile("V2-results.xml" , true, true);
