...
TypeId
NYUChannelConditionModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NYUChannelConditionModel")
    .SetParent<ChannelConditionModel> ()
    .SetGroupName ("Propagation")
    .AddAttribute ("UpdatePeriod", "Specifies the time period after which the channel condition is recomputed. If set to 0, the channel condition is never updated.",
                   TimeValue (MilliSeconds (0)),
...
void NYUChannelConditionModel::DoDispose ()
{
  m_channelConditionMap.clear ();
  m_updatePeriod = Seconds (0.0);
}
...
TypeId
NYUUMiChannelConditionModel::GetTypeId (void)
{
                                          ...
                                          Ptr<const MobilityModel> b) const
{
  ...
  // NYU Squared Model : https://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=7999294 (Table I, Row 2)
  double pLos = 0.0;
  if (distance2D <= 22.0)
  {
    pLos = 1.0;
